...
public function foto_u($id, $imgdata) {
  ...
  $data = array('ft' => $imgdata['file_name'] );
...
public function foto_cd($id, $imgdata) {
  ...
  $data = array('cr' => $imgdata['file_name'] );
  $this->db->where('usuarios_id_usuarios', $id);
  $this->db->update('clientes', $data);
}
public function foto_cm($id, $imgdata) {
  ...
  $data = array('cm' => $imgdata['file_name'] );
...
public function foto_cl($id, $imgdata) {
  ...
  $data = array('cl' => $imgdata['file_name'] );
  $this->db->where('	id_solicitud', $id);
  $this->db->update('solicitudes', $data);
}
...
{
  $this->db->select('e.id_bancos,e.banco');
  $this->db->from('bancos e');
  $this->db->order_by("e.banco", "asc");
  $query = $this->db->get();
  if($query->num_rows() > 0 )
  {
    return $query->result();
